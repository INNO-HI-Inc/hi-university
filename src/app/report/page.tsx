  ...
  if (!data) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4">
        <p className="text-[15px] text-slate">분석 결과가 없습니다.</p>
        <Link href="/analyze" className="rounded-xl bg-primary px-5 py-2.5 text-[13px] font-bold text-white transition hover:bg-primary-dark">생기부 분석하기</Link>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-paper">
      ...
      <div className="mx-auto max-w-[1100px] px-4 md:px-6">
        ...
        <div className="mb-10 rounded-2xl p-8 md:p-10 text-center border-2 border-dashed border-primary/20 bg-primary/3">
          <p className="text-[13px] font-bold text-primary mb-2">AI 분석 리포트</p>
          <h3 className="text-[20px] md:text-[24px] font-extrabold text-ink mb-3">다른 생기부도 분석해보세요</h3>
          <p className="text-[14px] text-slate mb-6">새로운 생기부 텍스트를 입력하면 즉시 새 리포트가 생성됩니다</p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link href="/analyze" className="inline-flex items-center gap-2 rounded-xl bg-primary px-8 py-3.5 text-[15px] font-bold text-white shadow-[0_4px_16px_rgba(68,140,255,0.3)] transition hover:bg-primary-dark active:scale-[0.97]">
              새 분석 시작
            ...
            <Link href="/" className="text-[14px] font-medium text-slate hover:text-ink transition">홈으로</Link>
          </div>
        </div>
      </div>
    </div>
